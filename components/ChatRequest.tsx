...
const ChatRequest: React.FC <ChatRequestProps> = ({adminId}) => {
    const [chatReuest, setChatRequest] = useState<(Conversation | null)[]>();
    ...
    return (
      <div>
        {(!chatReuest || chatReuest.length < 1)? (
          ...
          <ul
            role="list"
            className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 pt-4 px-5"
          >
            {chatReuest?.map((data, index) => (
              <li
                key={index}
                ...
                <div>
                  <div className="-mt-px flex divide-x divide-gray-200">
                    <div className="flex w-0 flex-1">
                      <button
                        onClick={() =>
                          handleAcceptConversation(data?.uniqueId!)
                        }
